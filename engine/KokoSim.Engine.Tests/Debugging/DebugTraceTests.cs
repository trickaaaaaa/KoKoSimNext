...
public sealed class DebugTraceTests
{
    ...
    public void Trace_PitchCountMatchesResultAndFieldsAreCoherent()
    {
        ...
        foreach (var t in sink.Pitches)
        {
            ...
            // 打球データは「スイング接触の InPlay/Foul」のときだけ載る。バント/スクイズの InPlay は
            // BattedBall を生成しない（BuntResolver/SqueezeResolver は打球速度を持たない）ため観測も null。
            // よって「載っているなら接触打球で妥当な値」「見送り球には決して載らない」を検証する。
            if (t.ExitVelocityKmh is { } ev)
            {
                Assert.InRange(ev, 1.0, 250.0);
                Assert.True(t.Kind is PitchKind.InPlay or PitchKind.Foul);
            }
