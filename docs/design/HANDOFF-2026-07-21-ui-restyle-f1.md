...
> **【2026-07-21 追記】F1 の実装は完了し、ゲート（Go/NoGo）待ちの状態。**
> 以下の本文は着手前の指示書。実施結果は `design-16-ui-restyle.md` の §6 フェーズ表・§9 未決事項・§10（F1-d の穴リスト）を参照。
> 確定した未決事項: ScoreboardStrip は**全画面ヘッダー**／角丸は**カード6・モーダル8・パネル6・チップ2**／TopBar は**案B 升目グリッド**。
> 検証スクショ: `screenshots/f1-final-*.png`（8画面）と `screenshots/tournament-preview/*.png`（大会展望）。
> §0.5 のコミット確認は不要になった（F0＋ホーム改修は `61aaaa1` / `6a6d50c` でコミット済み）。
