...
| **詳細モードの停止粒度** | ~~打席頭のみ＋手動一時停止~~ → **連続自動進行＋タイム（伝令）中断**（issue #173 で改定） | ~~進行の自動停止は打席頭~~。**改定（issue #173・2026-07-24）**: 打席頭でも自動停止しない。試合は終局まで**連続自動進行**し、プレイヤーは**任意のタイミングで「タイム」（＝有限資源の伝令。design-09 §3・攻守各3回・延長+1）をかけて中断**し、采配一式（交代・サイン方針・伝令効果）を差し込んで「再開」する。design-09 §4 の高速モード像に一本化（UI原則⑦「操作は少なく深く」） |
...
| **Q12-2 手動停止の割り込み口** | **全球で采配ショートカット常駐**（→ issue #173 で「タイム中断中に設定」へ整理） | 進行は自動停止しない（issue #173 で打席頭停止も廃止＝連続自動進行）。1球采配のショートカット（打撃/配球）は常時 UI 上にあるが、連続進行では止まらないため実質的には**タイム中断中に次打席へ向けて設定する**運用になる（消費後は方針へ復帰＝1球限り。§2.3） |
